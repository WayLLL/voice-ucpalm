   ...
   */
    ...
    public static String encrypt(String content, String passwd) {
        try {
        	 ...
        	 * DES加密
        	 */
        	/*Cipher aesECB = Cipher.getInstance("DES/ECB/PKCS5Padding");
            SecretKeySpec key = new SecretKeySpec(passwd.getBytes(), "DES");
            aesECB.init(Cipher.ENCRYPT_MODE, key);*/
        	
            byte[] result = aesECB.doFinal(content.getBytes());
            String bTemp = new BASE64Encoder().encode(result);  
...
*/             
    ...
    public static String desEncrypt(String content, String pwdKey) throws GeneralSecurityException, UnsupportedEncodingException {
        
    	byte source[] = content.getBytes("UTF-8");
		byte rawKeyData[] = pwdKey.getBytes("UTF-8");
    ...
    public static byte[] desDecrypt(byte[] data, byte rawKeyData[]) throws GeneralSecurityException {
        // 处理密钥
        SecretKeySpec key = new SecretKeySpec(rawKeyData, "DES");
        // 解密
        Cipher cipher = Cipher.getInstance("DES/ECB/PKCS5Padding");
    ...
    public static void main(String[] args) {
         ...
         * 加密
         */
        /*System.out.println("使用AES对称加密，lpq请输入加密的规则");
		...
		/*try {
			String content ="cwk";
			String pwdKey = "lpq12345";
			System.out.println(se.desEncrypt(content, pwdKey));
