...
public class DateUtil {
	 ...
	 * 
	 * @param precision
	 *            -只能Calendar.YEAR/Calendar.MONTH/Calendar.DAY_OF_YEAR/Calendar.
	 *            DAY_OF_MONTH否则返回null
	 * @param amount
	 * @param d1
	 * @return
	 */
	...
	public static String date2TimeStamp(String dateStr, String format) {
		...
		if (StringUtil.isNotEmpty(dateStr)) {
			...
			try {
				Date date = sdf.parse(dateStr);
				timestamp = String.valueOf(date.getTime());// 到毫秒
			} catch (ParseException e) {
				e.printStackTrace();
			}
		}
		if (StringUtil.isNotEmpty(timestamp)) {// 时间戳到秒
